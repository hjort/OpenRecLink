...
//(*IdInit(execstd)
const long
  execstd::ID_TEXTCTRL2 = wxNewId();
const long
  execstd::ID_BITMAPBUTTON2 = wxNewId();
const long
  execstd::ID_TEXTCTRL4 = wxNewId();
const long
  execstd::ID_BITMAPBUTTON1 = wxNewId();
const long
  execstd::ID_STATICTEXT1 = wxNewId();
const long
  execstd::ID_STATICTEXT3 = wxNewId();
const long
  execstd::ID_GAUGE1 = wxNewId();
const long
  execstd::ID_STATICTEXT2 = wxNewId();
const long
  execstd::ID_STATICTEXT5 = wxNewId();
const long
  execstd::ID_BITMAPBUTTON3 = wxNewId();
const long
  execstd::ID_STATICLINE1 = wxNewId();
const long
  execstd::ID_STATICLINE2 = wxNewId();
const long
  execstd::ID_PANEL1 = wxNewId();
//*)

BEGIN_EVENT_TABLE(execstd, wxDialog)
  //(*EventTable(execstd)
  //*)
  END_EVENT_TABLE()execstd::execstd(wxWindow * parent, wxWindowID id, const wxPoint & pos,
				    const wxSize & size)
{
  //(*Initialize(execstd)
  wxFlexGridSizer *
  ...
  Create(parent, wxID_ANY, _("Executa padronização"), wxDefaultPosition, wxDefaultSize,
  ...
  Connect(ID_BITMAPBUTTON2, wxEVT_COMMAND_BUTTON_CLICKED,
	  (wxObjectEventFunction) & execstd::OnopencfgfileClick);
  Connect(ID_BITMAPBUTTON1, wxEVT_COMMAND_BUTTON_CLICKED,
	  (wxObjectEventFunction) & execstd::OnopenoutfileClick);
  Connect(ID_BITMAPBUTTON3, wxEVT_COMMAND_BUTTON_CLICKED,
	  (wxObjectEventFunction) & execstd::OnBitmapButton2Click);
  //*)
}
...
void
execstd::OnopenoutfileClick(wxCommandEvent & event)
{
  ntable t;
...
void
execstd::OnopencfgfileClick(wxCommandEvent & event)
{
  FileDialog1->SetWildcard(_T("*.stdcfg"));
...
execstd::exec(void)
{
  StopWatch1.Start();
  wxBeginBusyCursor();
  dbconv::doconv myconv(cfgfilenamectrl->GetValue(), outfilenamectrl->GetValue());
  if (myconv.init())
  {
...
void
execstd::OnBitmapButton2Click(wxCommandEvent & event)
{
  ...
  log.log_event(_T("STDX"), cfgfilenamectrl->GetValue(), timestring, msg);

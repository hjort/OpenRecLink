...
//*)
  BEGIN_EVENT_TABLE(execjin, wxDialog)
  //(*EventTable(execjin)
  //*)
  END_EVENT_TABLE()execjin::execjin(wxWindow * parent, wxWindowID id, const wxPoint & pos,
				    const wxSize & size)
{
  //(*Initialize(execjin)
  wxFlexGridSizer *
...
void
execjin::OnopenoutfileClick(wxCommandEvent & event)
{
  ntable t;
...
void
execjin::OnopencfgfileClick(wxCommandEvent & event)
{
  FileDialog1->SetWildcard(_T("*.jincfg"));
...
execjin::exec(void)
{
  ...
  if (myjoin.init(cfgfilenamectrl->GetValue(), outfilenamectrl->GetValue()))
  {
    int nrecs = myjoin.norecs();
...
void
execjin::OnBitmapButton2Click(wxCommandEvent & event)
{
  ...
  log.log_event(_T("JOINX"), cfgfilenamectrl->GetValue(), timestring, msg);
